...
static int lua_cocos2dx_runtime_addSearchPath(lua_State* tolua_S)
{
    int argc = 0;
...
static int lua_cocos2dx_runtime_setSearchPaths(lua_State* tolua_S)
{
    int argc = 0;
    cocos2d::FileUtils* cobj = nullptr;
    bool ok  = true;
...
void RuntimeLuaImpl::startScript(const std::string& strDebugArg)
{
    ...
    if (!strDebugArg.empty())
    {
        ...
        cc_luaopen_lua_debugger(engine->getLuaStack()->getLuaState());
