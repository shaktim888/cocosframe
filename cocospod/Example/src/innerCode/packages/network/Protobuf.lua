...
function decodeAll(typename, buffer, length)
	...
	if ret then
	  	decodeTable(ret,typename:sub(1,typename:match(".*%.()")-1))
	end
	return ret
end
 
function decodeTable(tbl,typename)
    for k,v in pairs(tbl) do
        if type(v) == "table" then
            if(type(v[1]) == "string" and string.find(v[1],typename)) then
                local ret = decode(v[1], v[2])
                if ret then
                    tbl[k] = ret
                end
            end
            decodeTable(tbl[k],typename)
        end
    end
end
